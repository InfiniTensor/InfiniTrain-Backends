...
void MacaGuardImpl::InitSingleHandle(Device device) {
  ...
  std::call_once(*g_device_stream_flags.at(device.index()), InitSingleStream,
                 device);

  g_maca_blas_handles[device.index()] =
      std::make_unique<MacaBlasHandle>(g_maca_streams[device.index()].get());
...
void MacaGuardImpl::Initialize() {
  std::call_once(initialize_flag_, [] {
    ...
    CHECK_LE(static_cast<size_t>(device_count),
             static_cast<size_t>(std::numeric_limits<int8_t>::max()) + 1)
...
int MacaGuardImpl::DeviceCount() const {
  return static_cast<int>(g_maca_streams.size());
}

Device::DeviceType MacaGuardImpl::Type() const {
...
Stream *MacaGuardImpl::CreateStreamWithPriority(Device device,
                                                int priority) const {
  CheckMacaDevice(device);
  int current_device = -1;
  MACA_CHECK(mcGetDevice(&current_device));
...
// event
void MacaGuardImpl::EventCreate(Event **event) const {
  *event = new MacaEvent();
}

void MacaGuardImpl::EventCreateWithFlags(Event **event, EventFlag flags) const {
  *event = new MacaEvent(flags);
}
...
void MacaGuardImpl::StreamWaitEvent(Stream *stream, Event *event,
...
RuntimeStatus MacaGuardImpl::EventSynchronize(Event *event) const {
  ...
  LOG(ERROR) << "MacaGuardImpl::EventSynchronize failed: "
...
RuntimeStatus MacaGuardImpl::EventQuery(Event *event) const {
  ...
  LOG(ERROR) << "MacaGuardImpl::EventQuery failed: "
             << mcGetErrorString(status);
  return RuntimeStatus::kError;
}

float MacaGuardImpl::EventElapsedTime(Event *start_event,
                                      Event *stop_event) const {
  auto start_maca_event = GetMacaEvent(start_event);
  auto stop_maca_event = GetMacaEvent(stop_event);
  float elapsed_ms = 0.0f;
  MACA_CHECK(
      mcEventElapsedTime(&elapsed_ms, start_maca_event, stop_maca_event));
  return elapsed_ms;
}
...
BlasHandle *MacaGuardImpl::GetBlasHandle(Device device) const {
  CheckMacaDevice(device);
  std::call_once(*g_device_handle_flags.at(device.index()), InitSingleHandle,
                 device);
...
void MacaGuardImpl::Memcpy(void *dst, const void *src, size_t count,
                           MemcpyKind kind) {
  if (kind == MemcpyKind::kH2D) {
    ...
    LOG(FATAL) << std::format("MacaGuardImpl::Memcpy got invalid MemcpyKind={}",
                              MemcpyKindToString(kind));
  }
}

void MacaGuardImpl::MemcpyAsync(void *dst, const void *src, size_t count,
  ...
  case MemcpyKind::kH2D:
    MACA_CHECK(
        mcMemcpyAsync(dst, src, count, mcMemcpyHostToDevice, maca_stream));
    break;
  case MemcpyKind::kD2H:
    MACA_CHECK(
        mcMemcpyAsync(dst, src, count, mcMemcpyDeviceToHost, maca_stream));
    break;
  case MemcpyKind::kD2D:
    MACA_CHECK(
        mcMemcpyAsync(dst, src, count, mcMemcpyDeviceToDevice, maca_stream));
    break;
  default:
    LOG(FATAL) << std::format(
        "MacaGuardImpl::MemcpyAsync got invalid MemcpyKind={}",
        MemcpyKindToString(kind));
  }
}
...
void RegisterMacaRuntime() {
  INFINI_TRAIN_REGISTER_DEVICE_GUARD_IMPL(Device::DeviceType::kPrivateUse1,
                                          MacaGuardImpl)
}
